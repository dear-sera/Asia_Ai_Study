...
form_window = uic.loadUiType('./mainWidget.ui')[0]  #QtDesigner에서 직접 만든 창을 가져오기

class Exam(QWidget, form_window):
    def __init__(self):
        super().__init__()  #super()는 상속받은 부모 클래스를 의미
        self.path = None
        self.setupUi(self)  #setupUi= Qt Designer를 통해 구성한 UI를 화면에 출력
        self.model = load_model('../models/horse_and_human_binary_classfication.h5')
    ...
    def predict_image(self):
        self.path = QFileDialog.getOpenFileName(
            self, #QFileDialog를 여는 것, "Open file"=QFileDialog의 이름, 그 다음''는 지정 경로, 그 다음 ''는 파일형태를 적은 것 대로 고를 수 있게 해준다
            "DataSets Files", "C:\work\python\AI_exam\datasets\horse_or_human",
            "Image Files(*.png);;Text Files(*.txt);;PDF Files(*.pdf);;All Files(*.*)"
        )
        print(self.path)  ##경로를 출력해주는 데 튜블로 나온다(처음 건 경로, 두번째껀 파일형태)
        if self.path[0]:  #경로를 받았을 때만 데이터를 선택할 수 있게
            ...
            #이미지 전처리
            try:
                img = Image.open(self.path[0])  #지정된 이미지 경로를 오픈
                img = img.convert('RGB')
            ...
            if predict_value > 0.5:  #0이면 말, 1이면 사람
