...
# 카테고리별 반복(2회), 첫 for문은 horse가 들어감 , idx = 인덱스번호가 들어감(horse은 0, human는 1)
for idx, category in enumerate(categories):  #enumerate= 자료형을 입력받으면 인덱스 값을 포함하는 enumerate 객체를 돌려줘서, horse, human을 하나씩 돌려준다
    files = glob.glob(img_dir + category + '*.png')  # 경로 내 해당 카테고리로 시작하는 파일 경로 리스트
...
np.save('../datasets/horse_human_image_data.npy', xy)